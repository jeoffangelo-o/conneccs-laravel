...
export default function AnnouncementFormScreen({ navigation }) {
  const { colors, isDark } = useTheme();
  const styles = createStyles(colors);
  const [formData, setFormData] = useState({ title: '', message: '', audience: '', priority: '' });
    ...
    <View style={styles.container}>
      ...
      <View style={styles.topbar}>
        ...
        <View style={styles.topbarCenter}>
          ...
          <Text style={styles.topbarBreadcrumb}>Create New Announcement</Text>
        </View>
        <View style={{ width: 24 }} />
      </View>
      ...
      <ScrollView style={styles.content}>
        <View style={styles.panel}>
          <View style={styles.panelHeader}>
            <Text style={styles.panelTitle}>Announcement Details</Text>
          </View>
          <View style={styles.panelBody}>
            <View style={styles.formGroup}>
              <Text style={styles.label}>Title *</Text>
              <TextInput style={styles.input} placeholder="Enter title" placeholderTextColor={colors.text3} value={formData.title} onChangeText={(text) => setFormData({...formData, title: text})} />
            </View>
            <View style={styles.formGroup}>
              <Text style={styles.label}>Message *</Text>
              <TextInput style={styles.textarea} placeholder="Write your message" placeholderTextColor={colors.text3} value={formData.message} onChangeText={(text) => setFormData({...formData, message: text})} multiline numberOfLines={6} textAlignVertical="top" />
            </View>
            <View style={styles.formGroup}>
              <Text style={styles.label}>Target Audience *</Text>
              <TextInput style={styles.input} placeholder="All Faculty" placeholderTextColor={colors.text3} value={formData.audience} onChangeText={(text) => setFormData({...formData, audience: text})} />
            </View>
            <View style={styles.formGroup}>
              <Text style={styles.label}>Priority Level *</Text>
              <TextInput style={styles.input} placeholder="High / Medium / Low" placeholderTextColor={colors.text3} value={formData.priority} onChangeText={(text) => setFormData({...formData, priority: text})} />
            </View>
